...
__shimcall__ INT32 OsAtomicAdd   (INT32 amount, PINT32 address)
{
	unsigned long flags;
...
__shimcall__ INT32 OsAtomicIncrement (PINT32 address){
	unsigned long flags;
	atomic_t *v = (atomic_t *)address;
	INT32 amount;
...
__shimcall__ INT32 OsAtomicDecrement (PINT32 address)
{
	unsigned long flags;
	atomic_t *v = (atomic_t *)address;
	INT32 amount;
...
__shimcall__ void OsThreadDestroy(POSTHRD osthrd)
{
	OsThreadStop(osthrd);
	OsFree(osthrd);
}

static void unwrap_kwork(struct kthread_work *work)
{
	struct kwork_data *w = container_of(work, struct kwork_data, work);
	w->func(w->data);
}

__shimcall__ void OsThreadScheduleInit(HOSSCHED hWorkStorage, __kernelcall__ void (*func)(void *), void * data)
{
...
__shimcall__ int OsThreadSchedule(POSTHRD osthrd, HOSSCHED hWorkStorage)
{
	bool ret;
	struct kwork_data *w = (struct kwork_data *)hWorkStorage;
...
__shimcall__ void OsThreadScheduleDone(void)
{
	OsModuleUseCountDec();
}
...
static INT32 ntimers = 0;

__shimcall__ HOSTIMER OsCreatePeriodicTimer(IN UINT32 InitialTimeOut, IN PCBFUNC pTimeOutCallBack,IN PALLOC_FUNC pFuncAlloc, IN PFREE_FUNC pFuncFree,IN PVOID pRefData, OUT HTHREAD* pThreadId)
{
...
__shimcall__ void OsDestroyPeriodicTimer(IN HOSTIMER hTimeOut)
{
