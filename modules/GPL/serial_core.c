...
*
*  $Id: serial_core.c,v 1.21 2001/11/23 12:55:15 rmk Exp $
*
* Slightly modified by Marc Boucher <marc@linuxant.com> to support virtual
* uarts (Conexant softmodems).
*
*/
...
static int uart_write(struct tty_struct *tty, int from_user,const u_char * buf, int count)
{
	struct uart_info *info = tty->driver_data;
	unsigned long flags;
	int c, ret = 0;
...
static int uart_set_info(struct uart_info *info,struct serial_struct *newinfo)
{
#ifndef SERIAL_IO_MEM
	return -ENOSYS;
#else
...
static void uart_close(struct tty_struct *tty, struct file *filp)
{
	...
	*/
	...
	spin_lock_irqsave(&info->lock, flags);
	if (
#ifdef NON_ATOMIC_TTY_STRUCT_COUNT
			(tty->count == 1)
#else
...
*/
SERIALCORE_EXPORT int uart_register_driver(struct uart_driver *drv)
{
	...
	*/
	...
	normal->driver_name	= drv->owner->name;
	...
	normal->init_termios.c_iflag = IGNPAR;
	normal->init_termios.c_oflag = 0;
	normal->init_termios.c_cflag = B115200 | CS8 | CREAD | HUPCL | CLOCAL | CRTSCTS;
	normal->init_termios.c_lflag = 0;
#ifdef TTY_DRIVER_NO_DEVFS
...
SERIALCORE_EXPORT void uart_unregister_driver(struct uart_driver *drv)
{
	int i;
...
*/
SERIALCORE_EXPORT int uart_register_port(struct uart_driver *drv, struct uart_port *port)
{
...
*/
SERIALCORE_EXPORT void uart_unregister_port(struct uart_driver *drv, int line)
{
	struct uart_state *state;
