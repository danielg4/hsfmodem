...
static int dcp_fasync(int fd, struct file *file, int on)
{
...
static unsigned int dcp_poll(struct file *file, poll_table * wait)
{
	dcp_unit_t *su = file->private_data;
	dcp_instance_t *pDcp = su->pDcp;
	unsigned int mask = 0;
...
static ssize_t dcp_read(struct file * file, char * buf, size_t nbytes, loff_t *ppos)
{
	dcp_unit_t *su = file->private_data;
	dcp_instance_t *pDcp = su->pDcp;
	ssize_t ret = 0;
...
static void call_dcp_daemon(int instNum, char *action)
{
	char *argv[3], *envp[6];
	int i=0;
	char devbuf[30];
...
__shimcall__ HANDLE DcpCreate(HANDLE hDevNode)
{
	unsigned long flags;
	dcp_instance_t *pDcp;
	char buf[32];
...
__shimcall__ void DcpSetVolume(HANDLE hDcp, int nVolume)
{
	dcp_instance_t *pDcp = hDcp;
	unsigned long flags;
...
__shimcall__ void DcpCallback(HANDLE hDcp, PVOID pData, UINT32 dwSize)
{
	dcp_instance_t *pDcp = hDcp;
	unsigned long flags;
	int s;
...
__shimcall__ void OsDcpExit(void)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,2)
	if (!IS_ERR(dcp_class))
...
__shimcall__ int OsDcpInit(void)
{
	int ret;
...
__shimcall__ void OsDcpEnsureDaemonIsRunning(int hwInstNum)
{
	struct list_head *e;
	unsigned long flags, flags2;
